...
# Stable marker embedded in every generated stub and checked by the
# production package loader (domain/package_runtime_loader.py).
SCAFFOLD_STUB_MARKER = "universal-agent: scaffold-stub"
SCAFFOLD_STUB_ENV = "UNIVERSAL_AGENT_ALLOW_SCAFFOLD_STUB"

# Renders Python source code (not HTML): autoescaping would corrupt the
# emitted module text. The sandboxed environment blocks unsafe template
# constructs; inputs come from the local `agent domain-packages scaffold`
# CLI invocation.
_TEMPLATE_ENV = SandboxedEnvironment(autoescape=False, lstrip_blocks=True)

_RUNTIME_STUB_TEMPLATE = _TEMPLATE_ENV.from_string(
    """# universal-agent: scaffold-stub - GENERATED by `agent domain-packages scaffold`.
# SCAFFOLD STUB: returns INCOMPLETE evaluations and scaffold-only tools.
# Replace this file with a real DomainRuntime implementation before any
# production use. Production loaders refuse scaffold stubs unless the
# environment variable UNIVERSAL_AGENT_ALLOW_SCAFFOLD_STUB is set to true.
