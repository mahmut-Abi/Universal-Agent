...
| Authorization model | Deferred | Runtime Policy owns action safety; bearer-token auth only gates HTTP access. Domain capability Policy is not user RBAC. | Define user/role/capability authorization, admin/operator/read-only scopes, and audit-visible denial reasons. **2026-09-20**: Phase 1 landed the RBAC core (`security.authorization.AuthorizationEvaluator`, roles admin/operator/read_only mapped to read_write/read_only scopes) enforced at the agentd auth boundary with auditable reasons (`rbac`, `cross_tenant`) distinct from runtime Policy denials; **Phase 2** added the admin plane (`/v1/admin/*` routes over `PostgresCredentialStore`: tenants/users/memberships/credentials, admin-role gated with legacy-token bootstrap) — capability-level RBAC and durable audit events remain Deferred. |
