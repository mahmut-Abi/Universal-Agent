...
| 项 | 说明 |
|---|---|
| 现状 | `persistence/sqlite.py` 已使用 SQLAlchemy Core `MetaData` / `Table` / `insert` / `select` / `update` / `Engine.begin()` 接管 session 与 runtime event 表定义、schema 创建、DML 构造和事务入口；`distributed/worker_state.py` 的 `SQLiteWorkerRegistry` 已用 SQLAlchemy Core 接管 worker registry 表定义、schema 创建、load/save DML 和连接入口，同时保留 `BEGIN IMMEDIATE` 串行化语义；公开 store/registry 接口与 `sqlite_transaction` state/event commit 策略保持不变 |
| 收益 | SQLite persistence 与 worker registry 的 schema/DML 从多段手写 SQL 字符串收口到 typed Core expression seam，减少字段漂移、参数顺序错误和事务入口分叉；重复 session 仍映射为现有 `ValueError`，重复 event 继续保留旧 sqlite integrity 异常兼容 |
| 剩余 | P6 `SQLiteWorkQueue`、`SQLiteDistributedLockRegistry` 仍是直接 `sqlite3`；生产数据库迁移/versioning 后续再评估 Alembic |
| 风险 | 低：persistence integration tests、worker registry / SQLite serialization unit tests、RuntimeHost/CLI SQLite 配置路径、ruff 和 mypy 已覆盖 |
...
| SQLAlchemy / Alembic | P6 SQLite coordination / production DB adapters | Runtime persistence 与 worker registry 第一批已用 SQLAlchemy Core；后续按存储 seam 迁移 queue/lock，并在生产数据库 adapter 出现时引入 Alembic migration | 避免一次性替换所有本地 coordination primitive，先保持 P6 测试语义稳定 |
...
第三批（体验与观测）
    ...
    filelock 文件协调锁（已完成）
    SQLAlchemy Core(SQLite Runtime persistence / worker registry 第一批已完成；P6 queue/lock 后续渐进)
