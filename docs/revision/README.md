...
- `docs/revision/2026-09-15-agents-md-scope-reconciliation.md` — UA-AUDIT-003 decision record: AGENTS.md roadmap vs implemented P0–P7 scope.
...
### [x] UA-AUDIT-002 — Remove domain imports from kernel-layer facade/evaluation dispatch
...
- Done when:
  - No kernel/runtime module in the main `universal_agent` package imports a concrete domain; profile -> service resolution lives in the domains composition layer (`universal_agent.domains.profile_service`).
  - Default domain resolution goes through profile/domain-package mechanisms; CLI shell and agentd route adapters remain legitimate composition surfaces (documented in the audit doc).
- Evidence: new `src/universal_agent/domains/profile_service.py` owns `build_configured_service` / `build_default_service` / `build_probe_service`; `facade.py` and `agentd/__main__.py` delegate lazily (agentd's duplicated third dispatch copy removed). Remaining concrete-domain imports outside `domains/` are only the CLI shell and the explicitly-named agentd kubernetes route adapter. Gates: `uv run pytest tests/integration/test_facade_golden_path.py tests/integration/test_p0_golden_path.py tests/integration/test_agentd_server.py tests/integration/test_agentd_kubernetes.py tests/integration/test_agentd_routes.py tests/unit/test_agentd_http_config.py -q` (72 passed); full suite `tests/ -q -n auto` exit 0; ruff + mypy --strict (416 files) clean.

### [x] UA-AUDIT-003 — Reconcile AGENTS.md roadmap with the implemented P0–P7 scope
...
- Evidence: combined decision implemented — `AGENTS.md` §13 gains a Scope status maturity matrix (P0–P3 stable / P3.5–P3.7 beta / P4–P7 experimental, frozen) and §19 gains a freeze note limiting existing experimental layers to bug/security/test upkeep; decision record with considered alternatives at `docs/revision/2026-09-15-agents-md-scope-reconciliation.md`.

### [x] UA-AUDIT-004 — Extend ErrorCode enum toward the AGENTS.md §9 categories
...
- Evidence: `ErrorCode` gains `TRANSIENT` / `PERMISSION_DENIED` / `DEPENDENCY_MISSING` / `USER_REQUIRED`; `FailureCategory.USER_REQUIRED` added; `classify_failure` maps all four to matching deterministic recovery categories; new `user_required_rule` (ASK_USER, priority 45) sits before the UNKNOWN catch-all in `default_recovery_rules`. Tests: `test_new_error_codes_classify_to_matching_categories` (parametrized) and `test_user_required_asks_user_deterministically` in `tests/unit/test_recovery_rules.py`. Gates: recovery/core suites 42 passed; full suite `tests/ -q -n auto` exit 0; ruff + mypy --strict (416 files) clean.

### [x] UA-AUDIT-005 — Add explicit approval/action binding negative test
...
- Evidence: `test_confirmation_binding_approval_does_not_authorize_other_mutations` in `tests/integration/test_p1_safety_invariants.py` — approving the restart executes only the bound restart (with its exact arguments); the follow-up scale re-enters Policy (require_confirmation) and pauses for its own approval; the second approval executes scale and never re-executes the restart. Gates: `uv run pytest tests/integration/test_p1_safety_invariants.py -q` (8 passed); full suite `tests/ -q -n auto` exit 0; ruff + mypy --strict clean.

### [x] UA-AUDIT-006 — Set per-module coverage baseline gate
...
- Evidence: `[tool.coverage.report] fail_under = 86` ratchet (measured 86.7% combined line+branch on 2026-09-15); CI's coverage step (`pytest --cov`) now fails below the floor. Policy documented in pyproject: raise on improvement, never lower. Per-module baselines deferred in favor of the simpler repo-wide ratchet; lowest-coverage areas (facade.py 66%, tui_app.py 64%, sdk.py 0%) are visible in coverage.json for targeted follow-up.

### [x] UA-AUDIT-007 — Confine domain-feature code to its own domain package

- Priority: P1
- Area: Architecture / CLI / agentd / Domains
- Source: `docs/revision/2026-09-15-strict-audit.md` (UA-AUDIT-007), author decision
- Why: Kubernetes feature code (CLI command, agentd routes, init options, remote forwarding, eval suite, OpenAPI metadata) lived outside `universal_agent/domains/kubernetes` (123 references in 17 files), violating AGENTS.md §11 domain-package independence.
- Done when:
  - No kubernetes references remain in src outside `universal_agent/domains/` (verified by grep; pyproject entry-point registrations are the single legitimate registration point).
  - Hosts consume domain features via entry-point contribution contracts in the kernel (`universal_agent/host_contracts.py`).
  - Package boundary and startup-weight contracts keep passing.
- Evidence: four entry-point groups (`cli_contributions`, `agentd_routes`, `default_domains`, `evaluation_suites`); domain-side modules `domains/kubernetes/{registration,agentd_routes,eval_suite}.py`; deleted `agentd/_routes_kubernetes.py` and `universal_agent_cli/remote/kubernetes.py`; `--kubernetes-default` generalized to `--default-domain NAME`. Full suite exit 0; ruff + mypy --strict (420 files) clean.

### [x] UA-AUDIT-008 — Enforce kernel-internal layering with a seam ledger

- Priority: P2
- Area: Architecture / Kernel / Testing
- Source: `docs/revision/2026-09-15-strict-audit.md` (UA-AUDIT-007 follow-up boundary re-check)
- Why: the kernel-internal subpackage layering (core -> support libs -> runtime -> service/host -> agentd) was enforced only informally; upward imports could accumulate silently.
- Done when:
  - A guard test scans every kernel subpackage pair and fails on any new upward import.
  - Every existing upward seam is listed with its rationale (sanctioned seam ledger), and a companion test fails when a listed seam disappears so the ledger stays truthful.
  - Known debts are documented: UA-D1 (profile -> host.config config-type placement), UA-D2 (persistence -> runtime.events helper placement).
- Evidence: `test_kernel_layering_has_no_new_upward_imports` + `test_sanctioned_layering_seams_still_exist` in `tests/unit/test_package_boundaries.py` (sanctioned ledger: 16 seams with rationale); full suite exit 0. Debts UA-D1/UA-D2 are open follow-ups with remediation plans in the audit doc.

### [x] UA-CS-001 — Thin-client config file + standalone Node.js web tier

- Priority: P1
- Area: Architecture / Client-Server / Web
- Source: author decision 2026-09-15 (client/server separation + Node.js web rebuild; P5 freeze exception recorded in `docs/revision/2026-09-15-agents-md-scope-reconciliation.md`)
- Why: the CLI bundled the full kernel and had no config-file way to point at a remote agentd; the web console was embedded in agentd.
- Done when:
  - `agent init --server-url/--server-auth-token-env` writes a `server` section into config.json; the CLI resolves the API target as flag > `AGENT_API_URL` env > config file, and thin-client mode becomes the default when a target is configured.
  - A standalone Node.js web tier (`web/`, zero npm deps) serves a multi-session chat UI, proxies `/api/*` to agentd with server-side token injection and SSE pass-through, deploys via its own compose service + Dockerfile.
  - Smoke tests cover config resolution, init payload, and the web proxy against a real Node process.
- Evidence: `tests/unit/test_client_config.py` (7 tests), `tests/integration/test_web_server.py` (4 tests vs live node process), full suite exit 0, ruff + mypy --strict clean.

### [x] UA-CS-002 — Remove the embedded web frontend package; web chat UX enhancements

- Priority: P2
- Area: Web / agentd
- Source: author decision (the interactive web tier is the standalone Node.js service; the agentd-embedded frontend package is obsolete)
- Done when:
  - `src/universal_agent_web/` is deleted; pyproject wheel packages and boundary tests updated.
  - agentd `/console` serves a built-in info page (API + standalone-web pointer); JSON projection and operator-action routes are unchanged.
  - Web chat adds: login gate (`WEB_PASSWORD` env, HttpOnly session cookie), session search filter, and periodic multi-session refresh.
- Evidence: `test_agentd_console.py` updated (info page semantics), `test_web_static.py` removed, `test_web_server.py` login-gate tests (block/401/wrong-password/cookie/grant); full suite exit 0; ruff + mypy --strict clean.

### [x] UA-CS-003 — Postgres store backend wiring + web session detail tabs

- Priority: P2
- Area: Architecture / Persistence / Web
- Source: UA-PROD-001 direction; author follow-up feature request
- Done when:
  - `StoreBackend.POSTGRES` + `StoreConfig.url_env` (DSN read from env at assembly time; never in config); `_build_stores` wires `PostgresRuntimeStore` lazily with a clear missing-extra error.
  - Web chat gains session detail tabs (Timeline / Evidence / World) over the agentd console JSON projections.
  - Compose gains an optional `postgres` profile service.
- Evidence: `tests/unit/test_postgres_store_wiring.py` (5 tests: parse, validate, env resolution precedes extra import); web tabs verified via `node --check`; full suite exit 0; ruff + mypy --strict clean.

### [x] UA-CS-004 — Fix TUI boundary violations (client must not import agentd adapter)

- Priority: P2
- Area: Architecture / TUI / Boundaries
- Source: author follow-up ("tui 呢，是不是也没处理") after the client/server split
- Why: `tui.py` imported the three read-model projections from `universal_agent.agentd.*` and `tui_app.py` imported `runtime_run_body` from `agentd.representations` — a client package importing the server application adapter. (An earlier audit had wrongly reported "TUI -> api only"; the scan pattern was too narrow.)
- Done when:
  - The three projection modules move to the kernel service layer (`universal_agent.service.{timeline,world_explorer,evidence_drilldown}`); agentd keeps no copies.
  - TUI builds the run body from core helpers (`to_json_object`) instead of `agentd.representations`.
  - The agentd-adapter guard test covers client packages (`universal_agent_tui`).
- Evidence: `test_runtime_layers_do_not_import_agentd_adapter` extended and passing; TUI/timeline/world/evidence/console suites + full suite exit 0; ruff + mypy --strict clean.

### [x] UA-P0-002 — Scaffold stub production guard

- Priority: P1
- Area: Domain SDK / Safety / CI
- Source: author P0 optimization list (scaffold stubs must be replaced before production; forbid stub loading on production paths)
- Done when:
  - Generated stubs carry a stable marker (`universal-agent: scaffold-stub`) and a "replace before production" header.
  - `load_domain_package_runtime` refuses scaffold stubs unless `UNIVERSAL_AGENT_ALLOW_SCAFFOLD_STUB=true` (dev-only escape hatch).
  - Tests cover refuse/allow/non-scaffold paths; scaffold rendering uses a sandboxed Jinja environment.
- Evidence: `test_production_loader_refuses_scaffold_stub`, `..._allows_scaffold_stub_when_explicitly_enabled`, `..._loads_real_runtimes_without_stub_env` in tests/unit/test_domain_package.py (36 passed); conftest defaults stubs to allowed for the wider suite; full suite exit 0; ruff + mypy --strict clean.

### [x] UA-P0-003 — Add production K8s scenarios (pending scheduling, readiness probes)

- Priority: P2
- Area: Kubernetes / Evaluation
- Source: author P0 optimization list (golden vertical: more real production scenarios)
- Done when:
  - Two new canonical scenarios: unschedulable Pending pods (scheduling pressure) and failing readiness probes after a bad rollout; both diagnosis-kind with evidence claims (deployment.status, pod.status) and bounded actions.
  - Scenario library test asserts the new names and config validity.
- Evidence: tests/scenarios/k8s-{pending-unschedulable,readiness-probe-failing}.json; test_scenario_library.py 5 passed; full suite exit 0. Live-kind expansion of the gated contract test is queued next (requires a reachable cluster to validate).

### [x] UA-P0-004 — Split the three largest kernel modules by responsibility

- Priority: P2
- Area: Maintenance / Runtime / Distributed / Multi-Agent
- Source: author P0 optimization list (reduce cognitive load of oversized files)
- Done when:
  - `multi_agent/orchestrator.py` (732) -> orchestrator (549) + `delegation_state.py` (252: ledger types + payload codec + typed errors).
  - `distributed/queue.py` (935) -> `queue.py` shim (40) + `queue_memory.py` (397) + `queue_file.py` (235) + `queue_sqlite.py` (350).
  - `runtime/agent.py` (1045) -> 1015 + `runtime/recovery_planning.py` (recovery planning as explicit-dependency functions).
  - Public import surfaces unchanged; full suite green after each split.
- Evidence: full suite exit 0 after each split; ruff + mypy --strict clean; boundary + startup-weight contracts pass.

### [x] UA-CS-005 — Config-management write surface (profile CRUD + validate + audit)

- Priority: P2
- Area: agentd / Config Management / Web
- Source: author REST write-contract proposal (profiles CRUD, config validate, domain bindings, audit trail)
- Done when:
  - Kernel `ProfileStore` (profile/store.py): directory-backed CRUD with validate-before-persist, RFC 7386 patch semantics, domain binding rules (primary domain protected, dual-list update), JSONL config-audit log.
  - agentd routes: POST/PATCH/DELETE /v1/profiles(/{name}), POST /v1/config/validate, PUT /v1/domains/{name}/profiles, GET /v1/config/audit; wired via `--profiles-dir`.
  - Structured field-level validation errors (400 with error.errors[]); actor from X-Acting-Principal header.
- Evidence: tests/unit/test_profile_store.py (14), tests/integration/test_agentd_config_admin.py (7); full suite exit 0; ruff + mypy --strict clean. Policies CRUD deferred (needs kernel design for config-declared declarative policies); runtime read models stay authoritative for loaded profiles (apply on restart/reload).

### [x] UA-CS-006 — LLM call capture + trace detail + status code alignment

- Priority: P1
- Area: Runtime / agentd / Web detail views
- Source: author backend gap list (traces?detail=full, llm-calls, 422/204/409-builtin)
- Done when:
  - ModelUsage gains prompt/completion (redacted + bounded to 4000 chars); adapters capture at call time.
  - Runtime emits LLMCallRecorded event; ActionCompleted carries bounded redacted input/output.
  - GET /v1/sessions/{id}/llm-calls + GET /v1/sessions/{id}/traces?detail=full (joins LLM calls + I/O).
  - Config plane: validation → 422 {errors[]}, DELETE → 204, builtin profiles → 409 (ProfileBuiltinError).
- Evidence: test_model_http.py updated (prompt/completion assertions), test_agentd_session_messages.py (3), test_agentd_config_admin.py (7 incl. builtin 409); full suite exit 0; ruff + mypy --strict clean.
...
2026-09-15 UA-AUDIT-002 completed: kernel-layer concrete-domain imports removed. New domains-package composition point `universal_agent/domains/profile_service.py` owns the profile->service dispatch (`build_configured_service`, `build_default_service`, `build_probe_service`); `facade.py` and `agentd/__main__.py` delegate lazily and agentd's duplicated third dispatch copy is gone. Remaining concrete-domain imports outside `domains/` are only the CLI shell and the agentd kubernetes route adapter (documented as legitimate composition surfaces in docs/revision/2026-09-15-strict-audit.md). Evidence: targeted suites (facade/p0/agentd server+routes+kubernetes/http config) 72 passed; full suite `tests/ -q -n auto` exit 0; `uv run ruff check src tests` clean; `uv run mypy src tests` clean (416 files). Main files: src/universal_agent/domains/profile_service.py, src/universal_agent/facade.py, src/universal_agent/agentd/__main__.py.
2026-09-15 UA-AUDIT-004 completed: extended ErrorCode toward the AGENTS.md section 9 categories - added TRANSIENT / PERMISSION_DENIED / DEPENDENCY_MISSING / USER_REQUIRED, FailureCategory.USER_REQUIRED, classify_failure mappings for all four, and the deterministic user_required_rule (ASK_USER, priority 45) ahead of the UNKNOWN catch-all. Tests: parametrized classification test plus user-required dispatch test in tests/unit/test_recovery_rules.py (recovery/core suites 42 passed); full suite exit 0; ruff + mypy --strict clean. Main files: src/universal_agent/core/models.py, src/universal_agent/recovery/{models.py,manager.py,rules.py}, tests/unit/test_recovery_rules.py.
2026-09-15 UA-AUDIT-003 completed: reconciled AGENTS.md with the implemented P0-P7 scope via a combined decision. AGENTS.md section 13 gains a Scope status maturity matrix (P0-P3 stable, P3.5-P3.7 beta, P4/P5/P6/P7 experimental and frozen) and section 19 gains a freeze note limiting existing experimental layers to bug fixes, security fixes, and test upkeep. Decision record with considered alternatives: docs/revision/2026-09-15-agents-md-scope-reconciliation.md. Main files: AGENTS.md, docs/revision/2026-09-15-agents-md-scope-reconciliation.md.
2026-09-15 UA-AUDIT-006 completed: coverage report-only became an enforced ratchet. `[tool.coverage.report] fail_under = 86` documents the measured 86.7% combined line+branch baseline (2026-09-15, 25.9k statements); CI's coverage step now fails below the floor; policy is raise-on-improvement, never lower. Per-module baselines deferred in favor of the repo-wide ratchet; lowest-coverage areas are recorded for targeted follow-up. Main files: pyproject.toml.
2026-09-15 UA-AUDIT-007 completed: confined all Kubernetes feature code to `universal_agent/domains/kubernetes`. Introduced kernel contribution contracts (`universal_agent/host_contracts.py`) plus four entry-point groups (`universal_agent.cli_contributions` / `agentd_routes` / `default_domains` / `evaluation_suites`) so the CLI and agentd hosts discover domain features without naming any concrete domain; moved the kubernetes CLI command/init options/remote forwarding/probe builder into `domains/kubernetes/registration.py`, the HTTP routes+OpenAPI metadata into `domains/kubernetes/agentd_routes.py`, and the eval suite into `domains/kubernetes/eval_suite.py`; deleted `agentd/_routes_kubernetes.py` and `universal_agent_cli/remote/kubernetes.py`; generalized `--kubernetes-default` to `--default-domain NAME`. Kubernetes references outside domains/: 123 -> 0. Full suite exit 0; ruff + mypy --strict (420 files) clean. Main files: src/universal_agent/host_contracts.py, src/universal_agent/domains/kubernetes/{registration,agentd_routes,eval_suite}.py, src/universal_agent_cli/{contributions.py,__init__.py,init.py,embedded.py,io.py,remote/client.py,parser/*}, src/universal_agent/agentd/{app.py,openapi.py,contributions.py,__main__.py}, pyproject.toml.
2026-09-15 UA-AUDIT-008 completed: kernel-internal layering is now enforced by test. `test_kernel_layering_has_no_new_upward_imports` scans every kernel subpackage pair and fails on new upward imports; `test_sanctioned_layering_seams_still_exist` keeps the sanctioned-seam ledger (16 entries with rationale) truthful. Boundary re-check also verified: optional-dep seams (starlette/uvicorn only in agentd; openai/psycopg/textual lazily in their packages), TUI -> api only, web static-only, no cross-domain imports, root package light. Two pre-existing debts documented as UA-D1 (profile -> host.config) and UA-D2 (persistence -> runtime.events) with remediation plans. Main files: tests/unit/test_package_boundaries.py.
2026-09-15 UA-D1/UA-D2 resolved: kernel-internal layering debts fixed. UA-D1: config types (RuntimeConfig/DomainConfig/StoreConfig/ModelConfig/...) moved to root-level `universal_agent/configuration.py` (core-only deps); `host/config.py` is a compat re-export shim; all 11 consumer imports migrated. UA-D2: event-stream plumbing (EventSink/EventReader/EventWatcher protocols, InMemoryEventSink, filter_events/poll_event_reader/heartbeat_event) moved to root-level `universal_agent/eventstream.py`; runtime/persistence/agentd/evaluation/operations/CLI consumers migrated and `runtime/__init__` keeps public-name re-exports. Sanctioned seam ledger shrunk accordingly (profile->host and persistence->runtime resolved; stale-seam guard verified). Full suite exit 0; ruff + mypy --strict (421 files) clean. Main files: src/universal_agent/{configuration.py,eventstream.py}, src/universal_agent/host/config.py, tests/unit/test_package_boundaries.py.
2026-09-15 UA-CS-001 completed: client/server separation + standalone web tier. Client: `agent init --server-url/--server-auth-token-env` writes the `server` section into config.json; the CLI resolves the API target as --api-url flag > AGENT_API_URL env > config file (`universal_agent_cli/client_config.py`) and defaults to thin-client mode when a target exists (embedded runtime becomes the local fallback). Server: existing agentd Docker image + compose service. Web: new standalone `web/` Node.js app (zero npm deps) — multi-session chat UI over the agentd API via an /api proxy with server-side bearer-token injection and SSE pass-through; `web/Dockerfile` (node:22-alpine, numeric UID) + compose `web` service. P5 freeze exception recorded in the reconciliation doc. Tests: test_client_config.py (7), test_web_server.py (4 vs live node). Full suite exit 0; ruff + mypy --strict clean. Main files: web/{server.mjs,public/*,Dockerfile,README.md}, compose.yaml, src/universal_agent_cli/{client_config.py,__init__.py,init.py,parser/golden_path.py}, tests/{unit/test_client_config.py,integration/test_web_server.py}.
2026-09-15 UA-CS-002 completed: removed the embedded `universal_agent_web` frontend package (interactive UI is the standalone web tier); agentd `/console` now serves a built-in info page while keeping all JSON projection + operator-action routes. Web chat UX: login gate (`WEB_PASSWORD` env -> HttpOnly SameSite=Strict session cookie), session search filter, and periodic sidebar refresh for live multi-session view. Tests: console info-page semantics, login gate block/401/wrong-password/cookie/grant. Full suite exit 0; ruff + mypy --strict clean. Main files: src/universal_agent/agentd/console_routes.py, web/{server.mjs,public/*}, pyproject.toml, tests/unit/{test_agentd_console.py,test_package_boundaries.py}, tests/integration/{test_web_server.py,test_agentd_routes.py,test_agentd_server.py}.
2026-09-15 UA-CS-003 completed: Postgres store backend wiring (StoreBackend.POSTGRES + StoreConfig.url_env — DSN read from env at assembly time, never in config; lazy PostgresRuntimeStore import with missing-extra guidance; compose optional `postgres` profile) and web session detail tabs (Timeline/Evidence/World over agentd console JSON projections via the /api proxy). Tests: test_postgres_store_wiring.py (5). Full suite exit 0; ruff + mypy --strict clean. Main files: src/universal_agent/{configuration.py,host/runtime.py}, web/public/{index.html,app.js,style.css}, compose.yaml, tests/unit/test_postgres_store_wiring.py.
2026-09-15 UA-CS-004 completed: TUI boundary violations fixed. The three read-model projections (timeline/world_explorer/evidence_drilldown) moved from `universal_agent.agentd.*` to the kernel service layer (`universal_agent.service.*` — deps are core/runtime/service.views only); `tui_app.py` builds the chat run body from core helpers instead of `agentd.representations.runtime_run_body`; the agentd-adapter guard test now covers the TUI client package. Also confirmed: remote TUI (`agent tui`) rides the unified config resolution in run_cli (config server.url / AGENT_API_URL -> AgentdClient), and TUI kernel imports (service/runtime/operations) are the documented fat-client seam pending the client-package split (UA 后续). Full suite exit 0; ruff + mypy --strict clean. Main files: src/universal_agent/service/{timeline,world_explorer,evidence_drilldown}.py, src/universal_agent/agentd/console_routes.py, src/universal_agent_tui/{tui.py,tui_app.py}, tests/unit/test_package_boundaries.py.
2026-09-15 UA-P0-002 completed: scaffold stub production guard. Generated stubs embed the stable marker `universal-agent: scaffold-stub` plus a replace-before-production header; `load_domain_package_runtime` reads the entrypoint module and refuses scaffold stubs with an actionable message unless UNIVERSAL_AGENT_ALLOW_SCAFFOLD_STUB=true; tests/conftest.py defaults the env to allowed so the wider suite keeps loading stubs, and three dedicated tests (refuse/allow/non-scaffold unaffected) verify the guard. Scaffold rendering switched to jinja2.sandbox.SandboxedEnvironment (generates Python source, not HTML). Full suite exit 0; ruff + mypy --strict clean. Main files: src/universal_agent/domain/{package_runtime_stub.py,package_runtime_loader.py}, tests/conftest.py, tests/unit/test_domain_package.py.
2026-09-15 UA-P0-003 completed: two production-grade canonical scenarios added — k8s-pending-unschedulable (Pending/Unschedulable scheduling pressure diagnosis) and k8s-readiness-probe-failing (post-rollout readiness probe failure diagnosis), both diagnosis-kind with bounded actions and deployment.status/pod.status evidence claims. Scenario library test extended. Live-kind contract expansion queued next (needs reachable cluster). Main files: tests/scenarios/{k8s-pending-unschedulable,k8s-readiness-probe-failing}.json, tests/integration/test_scenario_library.py.
2026-09-15 UA-P0-004 completed: the three largest kernel modules split by responsibility. multi_agent/orchestrator.py (732 -> 549 + delegation_state.py 252: ledger contracts + payload codec + typed errors); distributed/queue.py (935 -> shim 40 + queue_memory 397 + queue_file 235 + queue_sqlite 350, public names unchanged via the shim); runtime/agent.py (1045 -> 1015 + recovery_planning.py: recovery planning extracted as explicit-dependency functions (components/events/settle injected), with thin private delegators preserving internal call sites). Public import surfaces unchanged throughout; full suite exit 0 after each split; ruff + mypy --strict clean. Main files: src/universal_agent/multi_agent/{orchestrator,delegation_state}.py, src/universal_agent/distributed/queue{,_memory,_file,_sqlite}.py, src/universal_agent/runtime/{agent,recovery_planning}.py.
2026-09-15 UA-CS-005 completed: config-management write surface. Kernel ProfileStore (profile/store.py): directory-backed profile CRUD with validate-before-persist (ProfileStoreValidationError carries structured field errors), RFC 7386 JSON Merge Patch for PATCH, domain binding via dual-list update (top-level domains + runtime configured domains, primary domain protected from unbind), JSONL config audit log (actor/action/resource/details; credential values impossible by pattern). agentd config_admin_routes.py: POST /v1/profiles (201/409), PATCH/DELETE /v1/profiles/{profile}, POST /v1/config/validate (dry-run, field errors), PUT /v1/domains/{name}/profiles, GET /v1/config/audit; dispatched before static GET routes so runtime read models stay authoritative for loaded profiles; actor via X-Acting-Principal header; wired through `agentd serve --profiles-dir`. Policies CRUD deferred (kernel design needed for config-declared declarative policies). Docs: docs/config-admin-api.md. Tests: test_profile_store.py (14), test_agentd_config_admin.py (7); full suite exit 0; ruff + mypy --strict clean. Main files: src/universal_agent/profile/store.py, src/universal_agent/agentd/{config_admin_routes.py,app.py,__main__.py}, docs/config-admin-api.md.
2026-09-15 UA-CS-006 completed: LLM call capture + trace detail + status code alignment. ModelUsage gains bounded prompt/completion (4000-char cap, redacted via redact_sensitive_mapping); json_http + both openai adapters capture at decide time; runtime emits LLMCallRecorded event; ActionCompleted carries bounded redacted input/output/error. agentd: GET /v1/sessions/{id}/llm-calls (llm_calls_body projection with safe int conversion), GET /v1/sessions/{id}/traces?detail=full (spans + llm_calls + detail marker). Config plane: validation errors -> 422 {status:error, errors:[{path,message}]}, DELETE -> 204, builtin profiles (builtin:true) refuse PATCH/DELETE with 409 builtin_profile. Main files: src/universal_agent/model/{adapter,json_http,openai_adapters}.py, src/universal_agent/runtime/{agent,actions}.py, src/universal_agent/operations/llm_calls.py, src/universal_agent/agentd/{_routes_session,config_admin_routes}.py, src/universal_agent/profile/store.py.
2026-09-15 AGENTS.md updated to reflect the session's architecture decisions: section 11 gains Domain Contribution Contracts (entry-point groups, host neutrality, client packages must not import agentd adapters, boundary guards) and a Client/Server Boundary subsection (thin-client target resolution order, env-name-only credentials, standalone web tier); section 12 lists the stable host-boundary modules (host_contracts/configuration/eventstream/profile_service/service projections); section 13 scope status gains deployment notes (Docker agentd + optional postgres profile + standalone web tier as the recorded P5 freeze exception); section 22 DoD requires boundary-guard test updates when boundaries change.
2026-09-15 UA-AUDIT-005 completed: added the spec section 12 approval/action binding negative test `test_confirmation_binding_approval_does_not_authorize_other_mutations` to the I1-I10 invariant suite. Approving a restart executes only the bound restart proposal (exact capability and arguments); a follow-up scale decision does not inherit the approval - it re-enters Policy (require_confirmation) and pauses for its own confirmation; the second approval executes the scale and never re-executes the restart. Evidence: `uv run pytest tests/integration/test_p1_safety_invariants.py -q` (8 passed); full suite `tests/ -q -n auto` exit 0; ruff + mypy --strict clean. Main files: tests/integration/test_p1_safety_invariants.py.
```
