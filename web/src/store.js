...
export const OPS_VIEWS = { eval: '评估工作台', cluster: '分布式集群', memory: '记忆管理', cost: '成本分析', logs: '日志与追踪', k8sops: 'K8s 运维', ecosystem: '生态与包', audit: '审计中心', multiagent: '多智能体', health: '健康中心' }
...
export function statusCls(s) { return (STATUS_MAP[normStatus(s)] || ['st-paused'])[0] }
export function statusLabel(s) { return (STATUS_MAP[normStatus(s)] || [null, s])[1] }
...
export const activity = computed(() => m.activity || { values: [0, 0, 0, 0, 0, 0, 0], days: [], max: 0 })
export function doRefresh() { reload('overview') }
...
export const chatSessions = computed(() => m.sessions.map((s) => ({ id: s.id, title: s.goal, profile: s.profile, time: s.raw?.created_at ? new Date(s.raw.created_at).toTimeString().slice(0, 5) : '—', msgs: chatMsgs(s) })))
...
export const activeChat = computed(() => chatSessions.value.find((c) => c.id === activeChatId.value) || null)
...
export const filteredChats = computed(() => chatSessions.value.filter(c => chatFilter.value === '全部' || c.profile === chatFilter.value))
export function scrollChat() { if (chatMsgsEl.value) chatMsgsEl.value.scrollTop = chatMsgsEl.value.scrollHeight }
export function newChat() { toast('输入第一条消息后将创建真实会话（POST /v1/sessions）') }
...
export const pmForm = reactive({ name: '', desc: '', domains: [],
  model: { provider: 'scripted', name: '', endpoint: '', api_key_env: '', timeout_seconds: 30 } })
export const MODEL_PROVIDERS = ['scripted', 'json_http', 'openai_chat_completions', 'openai_responses']
...
export function askDelete(name) { delModal.name = name; delModal.open = true }
...
export const domainDetail = computed(() => domainModal.index >= 0 ? m.domains[domainModal.index] : null)
export function openDomainDetail(i) { domainModal.index = i; domainModal.open = true }
...
export const memList = computed(() => m.memories.filter(x => !memSearch.value || x.text.includes(memSearch.value)))
...
/* ── 成本 / 日志 / K8s / 生态 ── */
export const costMax = computed(() => Math.max(...m.cost.byModel.map(x => x[1])))
...
export const auditActs = computed(() => ['全部', ...new Set(m.audit.items.map(x => x.act))])
export const auditFiltered = computed(() => m.audit.items.filter(x =>
...
export function onOverlayClick(e) { if (e.target === e.currentTarget) closeModal() }
...
export { API_BASE, createState, normStatus, STATUS_MAP, loadOverview, apiLoadSessions, loadMetrics, loadSessionDetail, loadConfig, loadEval, loadCluster, loadMemory, memoryAdd, memoryRemove, loadCost, loadLogs, loadK8sOps, loadEcosystem, loadAudit, loadMulti, loadHealth, loadModelInfo, loadRuntimeConfig, createSession, sendMessage, pauseSession, resumeSession, apiCancelSession, profileCreate, profilePatch, profileRemove, putConfig, runEval }