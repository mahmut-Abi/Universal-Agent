...
def build_service() -> RuntimeService:
    ...
    runtime = AgentRuntime(
        model=ScriptedModelAdapter(
            [
                Decision(DecisionType.WAIT, "pause before distributed worker resume"),
                Decision(
...
async def main() -> None:
    ...
    waiting = await service.run_goal(
        Goal("Verify workload through distributed worker", (SuccessCriterion("healthy", True),)),
    ...
    print(f"scheduled={scheduled.scheduled_work_item.work_item_id if scheduled else None}")
    print(f"worker={worker.status.value if worker else None}")
